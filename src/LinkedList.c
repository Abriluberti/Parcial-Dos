...
void precioFinal(void *elemento) {
	Servicio *this = elemento;
	float total;
	...
	if (this != NULL && fn != NULL) {
		...
		if (listaFiltrada != NULL) {
			for (i = 0; i < len; i++) {
				...
				elementoFiltrado = ll_get(this, i);
				if (fn(elementoFiltrado)) {
					ll_add(listaFiltrada, elementoFiltrado);
